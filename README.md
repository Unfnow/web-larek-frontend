...
## Класс CartModel
Наследуется от интерфейса Cart. Абстрактный Класс описывает структуру заказа пользователя, который в последствии отправится на сервер. 
Поля:
...
Методы:
+ constructor():obj-Инициализирует пустые значения и возвращает объект. Класс является абстрактным, поэтому везде где он вызывается, используется один и тот же экземпляр
...
## Класс DataManager
Базовый класс. Отправляет, при помощи класса Api, Get-запрос на сервер и получает список товаров, после чего создаёт для каждого элемента списка экземпляр класса  ProductModel.
Методы:
+ dataFetcher(uri: string):obj- создаёт get-запрос для получения данных с сервера, получает на вход путь к директории. Возвращает json в виде объекта
+ dataParcer(elems:[obj]):void- переносит данные с сервера в память приложения при помощи создания экземпляра класса ProductModel для каждого товара. Принимет массив объектов json. Возвращает void.
Поля:
+ constructor(baseUrl: string, options: RequestInit = {}):void-принимает базовый URL и глобальные опции для всех запросов
+ direction: string- путь к директории в виде строки

## Класс OrderManager
Базовый класс. Вносит изменения в класс CartModel. Удаляет элементы из корзины, отправляет post-запросы заказов на сервер и очищает её после отправки запроса, если он успешен
Методы:
+ constructor(cart:obj):void- получает на вход экземпляр класса CartModel и инициализирует его. Возвращает void
+ cartDelete(elem:obj)-:void Принимает на вход объект и удаляет его из корзины.
+ cartClear():void- очищает данные всех полей экземпляра класса.
+ orderPost(cart:obj, uri: string):void- Отправляет заказ на сервер в виде post-запроса при помощи метода post() класса API. Принимает два параметра: экземпляр класса CartModel и путь к директории, куда необходимо отправить данные
...
## Класс AuthForm
Базовый класс. Класс описывающий логику обработки форм. Не позволяет нажать на кнопку "Далее"/"Оплатить", если поля не заполнены, а также сверяет вводимые данные с паттерном
Поля:
+ topInput: input- данные верхней строки формы
+ bottomInput: input- данные нижней строки формы
Методы:
+ constructor(formData: obj):void- Принимает данные с форм в виде объекта и вносит их в переменные
+ patternPhoneCheck(input: input):boolean- метод проверяет вводимые данные и сверяет их с паттерном телефонного номера. Принимает input, возвращает boolean
+ patternMailCheck(input: input):boolean- метод проверяет вводимые данные и сверяет их с паттерном почты номера. Принимает input, возвращает boolean
+ fillcheck(topInput: input, bottomInput: input):boolean- метод проверяет заполнены ли оба поле перед нажатием на кнопку. Принимает обе строкиформы типа input, возвращает boolean
+ showError<error:T,place:K>:void- метод, отображающий ошибку, если проверка не пройдена. Это дженерик метод. Он принимает два значаения- сообщение об ошибке error типа T и местно где небходимо отобразить ошибку place типа K. Вызывается из методов проверок.
+ approveButton(topInput: input, bottomInput: input):boolean- обработчик нажатия кнопки "Далее"/"Оплатить", записывает данные в экземпляр класса OrderManager. Принимает обе строкиформы типа input, возвращает boolean
...
## Класс Component
Базовый класс отображения. Содержит основные методы, используемые для визуализации.
Методы:
...
Методы:
+ constructor(prod:obj):void- принимает экземпляр класса ProductModel и создаёт модальное окно, заполняя его данными.
+ openModal(popup: string):void-  реализует открытие модального окна при помощи обработчика события on класса EventEmitter
+ closeModal(popup: string):void- реализует закрытие модального окна при помощи обработчика события on класса EventEmitter при нажатии на кнопку закрытия
+ closeModalbyESC(popup: string):void- реализует закрытие модального окна при помощи обработчика события on класса EventEmitter при нажатии на клавишу ESC
+ closeOverlay(popup: string):void- реализует закрытие модального окна при помощи обработчика события on класса EventEmitter при нажаие на пустое пространство

## Класс Icon
Базовый класс. Создаёт модальное окно при нажатии.
+ constructor(mod:obj):void- принимает экземпляр класса ProductModel либо CartModel
+ createModal(mod:obj):void- при помощи класса Modal создаёт модальное окно для карточки. Вызывается в теле конструктора. принимает экземпляр класса ProductModel либо CartModel
...
## Класс SuccessForm
Наследник класса Modal и интерфейса BaseForm. Конструктор класса выводит картинку и надпись об успешной оплате товара. Класс наследуется от класса Modal и при вызове единственной кнопки использует тот же функционал, что и при нажатии на "крестик". После этого метод orderPost() класса OrderManager делает post-запрос на сервер и при успешном действии вызывает метод cartClear(), который очищает корзину.
Методы:
+ constructor():void- Входных значений нет. Отображает модальное окно. Возвращает void.
...
+ mod:obj / объект класса CartModel или ProductModel

## События
+ GET_FORM- реагирует на ввод в форму
+ PAY- реагирует на нажатие кнопки "Оплатить"
+ NEXT- реагирует на нажатие кнопки "Далее"
+ ICON_TOUCH- реагирует на нажатие иконки
+ CROSS_CLOSE- реагирует на закрытие модального окна при нажатии на крестик
+ OVERLAY_CLOSE- реагирует на закрытие модального окна при нажатии на оверлей
+ ESC_CLOSE- реагирует на закрытие модального окна при нажатии на клавишу ESC
+ CART_CHANGE- реагирует на изменение корзины
