...
Методы:
+ constructor(formData: obj):void- Принимает даннве с форм в виде объекта и вносит их в переменные
...
+ createModal(prod:obj):void- при помощи класса Modal создаёт модальное окно для карточки. Вызывается в теле конструктора. Принимает экземпляр класса ProductModel
...
+ createModal(elem:obj):void- при помощи класса Modal создаёт модальное окно для карточки. Вызывается в теле конструктора. Принимает экземпляр класса CartModel
...
on change- для корзины(класс: CartModel)
...
Также стоит добавить, что какие-то части документации будут меняться в процессе разработки, так как, при реализации вывявляются ошибки текущий документации и идеиеё изменения.