...
+ orderPost(cart:obj, uri: string)- Отправляет заказ на сервер в виде post-запроса при помощи метода post() класса API. Принимает два параметра: экземпляр класса CartModel и путь к директории, куда необходимо отправить данные
...
Методы:
+ constructor(protected readonly container: HTMLElement)- принимает контейнер с HTML-кодом
+ toggleClass(element: HTMLElement, className: string, force?: boolean)- переключает класс. принимает  HTML-код, название класса и значение токена force.
+ setText(element: HTMLElement, value: unknown)- установливает текстовое содержимое. Принимает HTML-код и выводимый текст.
+ setDisabled(element: HTMLElement, state: boolean)- меняет статус блокировки. Принимает HTML-код и значение токена force.
+ setHidden(element: HTMLElement)- скрывает элемент. Принимает HTML-код
+ setVisible(element: HTMLElement)- показывает элемент. Принимает HTML-код
+ setImage(element: HTMLImageElement, src: string, alt?: string)- установает изображение с алтернативным текстом. Принимает HTML-код, ссылку на изображение и альтернативную подпись к картинке
...
+ createModal(prod:obj)- при помощи класса Modal создаёт модальное окно для карточки. Вызывается в теле конструктора. Принимает экземпляр класса ProductModel
